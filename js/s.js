function Streamium() {
    ...
    return d(window).scroll(d.throttle(250, b)), d(".subscriptio_list_product a").contents().unwrap(), d(".product-name a").contents().unwrap(), d(".subscriptio_frontend_items_list_item a").contents().unwrap(), d(".subscriptio_list_id a").contents().unwrap(), d(".product-thumbnail").remove(), d(".widget > ul").addClass("list-group"), d(".widget > ul > li").addClass("list-group-item"), d(".widget").fadeIn(), {
        ...
        slick: {
            ...
            infinite: !1,
            adaptiveHeight: !0,
            responsive: [{breakpoint: 480, settings: {appendArrows: !1, slidesToShow: 2, slidesToScroll: 2}}]
        },
        slickSeries: {
        ...
        init: function (a) {
            this.tileCount = this.isMobile.any() ? 2 : 6
        },
        isOdd: function (a) {
            return a % 2
        },
        limitWords: function (a, b, c) {
            var d = "", e = a.replace(/\s+/g, " "), f = e.split(" "), g = f.length, h = 0;
            if (g > b) {
        ...
        getData: function(a, b) {
            ...
            var _method = 'post', _url = streamium_object.ajax_url;
            switch(a.action) {
                case 'search_api_post':
                    _method = 'get';
                    _url = [streamium_object.ajax_url, a.action, a.search['s']].join('/') + '.json';
                    break;
                case 'recently_watched_api_post':
                case 'custom_api_post':
                    _method = 'get';
                    _url = [streamium_object.ajax_url, a.action].join('/') + '.json';
                    break;
                case 'tax_api_post':
                case 'home_api_post':
                    _method = 'get';
                    var _urlr = [streamium_object.ajax_url, a.action];
                    if(a.query && a.query['name'])
                        _urlr.push(a.query['name'].toLowerCase());
                    _url = _urlr.join('/') + '.json';
                    break;
        ...
        getData1: function (a, b) {
            a.query = streamium_object.query, a.search = streamium_object.search, d.ajax({
                type: "post",
                dataType: "json",
        ...
        recentTemplate: function (a, b) {
            var e = this, f = a.data, g = a.count;
            if (f.length > 0) {
                var h = "";
                for (i = 0; i < f.length; i++) h += this.caroTileTemplate(f, i, "recent");
                if (g < streamium_object.tile_count) for (c = 0; c < streamium_object.tile_count - g; c++) h += '<div class="tile filler"><div class="tile_inner-spacer"><div class="tile_inner"></div></div></div>'
            }
            d("#recently-watched").append('<section class="videos"><div class="container-fluid"><div class="row"><div class="col-sm-12"><div class="video-header"><h3>Recently Watched</h3></div></div></div><div class="carousels" id="recently">' + h + "</div></div></section>" + this.expandedTemplate("recent"));
            var j = d("#recently");
            j.slick(this.slick), j.find(".slick-prev").addClass("hidden"), j.on("setPosition", function (a, b, c) {
                d(this).find(".slick-active:first").addClass("far-left"), b.slideCount > 6 && d(this).find(".slick-active:last").addClass("far-right"), d(".lazy").Lazy(e.lazy)
            }), j.on("afterChange", function (a, b, c) {
        ...
        staticTemplate: function (a, b) {
            var c = a.data, e = a.count;
            if (this.isMobile.any() && (streamium_object.tile_count = 2), c.length > 0) {
                var f = "", g = 0;
                for (i = 0; i < c.length; i++) {
                    var h = i + 1, j = "tax-" + g;
                    i % streamium_object.tile_count == 0 && (f += '<div class="container-fluid"><div class="row static-row ' + (0 === i ? "static-row-first" : "") + '">'), f += this.staticTileTemplate(c, i, j, h);
                    var k = !1;
                    this.isMobile.any() ? this.isOdd(i) && (k = !0) : h % streamium_object.tile_count == 0 && (k = !0), (k || i === e - 1) && (f += "</div></div>" + this.expandedTemplate(j), g++)
                }
            }
            d("#" + a.id).html(f).promise().done(function () {
                d(".static-row .tile").css("width", 100 / streamium_object.tile_count + "%"), b(!0)
            })
        },
        ...
        getMovieData: function (a, b) {
            d(".series-watched-episode-title").empty();
            var c = this;
            var _method = 'post',
                _url = streamium_object.ajax_url;
            switch(a.action) {

                case 'streamium_get_dynamic_content':
                case 'streamium_get_dynamic_series_content':
                    _method = 'get';
                    _url = [streamium_object.ajax_url, a.action, a.post_id].join('/') + '.json';
            }

            d.ajax({
                url: streamium_object.ajax_url, type: "post", dataType: "json", data: a,
                crossDomain: true,
                headers : {
                    'Content-Type' : 'application/x-www-form-urlencoded; charset=UTF-8'
                },
                success: function (b) {
                    ...
                    var f = "." + b.cat, g = "#series-watched-caro-" + b.cat, h = "#series-watched-" + b.cat,
                        i = "#tile-white-selected-" + b.cat + "-" + a.post_id, j = b.title, k = d(f).width(),
                        l = Math.floor(k / 21 * 8);
                    ...
                    var m = Math.round(d(".cd-main-header").height()), n = Math.round(d(i).outerHeight()) + 4,
                        o = Math.round(d(f).offset().top);
                    d("html, body").animate({scrollTop: o - (n + m)}, 500), d(f).animate({height: l}, 250, function () {
                        d(f + " .s3bubble-details-inner-content").animate({opacity: 1}, 500, function () {
                        }), d(i).show(), d(i).addClass("tile-white-is-selected"), d('[data-toggle="tooltip"]').tooltip()
                    });
                    var p = d(f).next().find("div.series-watched-caro");
                    c.getData({
                        ...
                        var b = a.data, e = "", i = 0;
                        if (Object.keys(b).length > 0) for (var k = 0; k < Object.keys(b).length; k++) {
                            var l = b[k + 1];
                            if (l.length > 0) for (var m = 0; m < l.length; m++) {
                                var n = i++;
        ...
        homeTemplate: function (b, e) {
            var f = this, g = b.data;
            if (g.length > 0) for (a = 0; a < g.length; a++) {
                var h = g[a].meta.title, i = g[a].meta.name, j = g[a].meta.catSlug, k = g[a].meta.link,
                    l = (g[a].meta.home, g[a].meta.count), m = g[a].data;
                if (m.length > 0) {
                    for (var n = "", o = 0; o < m.length; o++) n += this.caroTileTemplate(m, o, j);
                    if (l < streamium_object.tile_count) for (c = 0; c < streamium_object.tile_count - l; c++) n += '<div class="tile filler"><div class="tile_inner-spacer"><div class="tile_inner"></div></div></div>';
        ...
        customTemplate: function (b, e) {
            var f = this, g = b.data;
            if (g.length > 0) for (a = 0; a < g.length; a++) {
                var h = (g[a].meta.name, g[a].meta.catSlug, g[a].meta.type), j = g[a].meta.link,
                    k = (g[a].meta.home, g[a].meta.taxUrl, g[a].meta.title), l = g[a].meta.count, m = g[a].data, n = "";
                for (i = 0; i < m.length; i++) n += this.caroTileTemplate(m, i, h);
                if (l < streamium_object.tile_count) for (c = 0; c < streamium_object.tile_count - l; c++) n += '<div class="tile filler"><div class="tile_inner-spacer"><div class="tile_inner"></div></div></div>';
                d("#custom-watched").append('<section class="videos"><div class="container-fluid"><div class="row"><div class="col-sm-12"><div class="video-header"><h3>' + k + '</h3><a class="see-all" href="' + j + '">View all</a></div></div></div><div class="carousels" id="custom-slick-' + a + '">' + n + "</div></div></section>" + this.expandedTemplate(h));
                var o = d("#custom-slick-" + a);
                o.slick(this.slick), o.find(".slick-prev").addClass("hidden"), o.on("setPosition", function (a, b, c) {
                    d(this).find(".slick-active:first").addClass("far-left"), b.slideCount > 6 && d(this).find(".slick-active:last").addClass("far-right"), d(".lazy").Lazy(f.lazy)
                }), o.on("afterChange", function (a, b, c) {
                    d(this).find(".tile").removeClass("far-left").removeClass("far-right"), d(this).find(".slick-active:first").addClass("far-left"), d(this).find(".slick-active:last").addClass("far-right"), 0 === c ? d(this).find(".slick-prev").addClass("hidden") : d(this).find(".slick-prev").removeClass("hidden"), b.currentSlide >= b.slideCount - b.options.slidesToShow ? d(this).find(".slick-next").addClass("hidden") : d(this).find(".slick-next").removeClass("hidden")
                })
            }
            e(!0)
...
if (function (a, b) {
    ...
    }(this), jQuery(document).ready(function (a) {
        function b() {
            g = Math.round(a(window).innerWidth() / 21 * 9), a(".streamium-slider .slick-slide").height(g)
        }

        var c = Streamium(), d = streamium_object.tile_count, e = Math.round(a(".container-fluid").width() / d),
            f = e / 2, g = Math.round(a(window).innerWidth() / 21 * 9);
        ...
        c.isMobile.any() && (h = "tile"), a("body").on("click", "." + h, function (b) {
            b.preventDefault();
            var d = a(this).data("cat"), e = a(this).data("id"), f = a(this).data("nonce");
            return a(".series-watched").fadeOut(), a(".tile-white-selected").hide(), a(".tile-white-selected").removeClass("tile-white-is-selected"), c.getMovieData({
            ...
            a(".series-watched").fadeOut(), a(".tile-white-selected").hide(), c.animate({opacity: 0}, 250, function () {
                c.parent().animate({height: 0}, 250, function () {
                })
            })
            ...
            c.getData({
                ...
                b.error && console.log("ERROR:", b.message), a(".streamium-review-panel-content").html(b.content), a(".streamium-review-panel-header h1").html(b.title), a(".streamium-review-panel").addClass("is-visible")
            })
        }), c.isMobile.any() || a("body").on("mouseenter", ".tile_inner-home", function () {
            if (1 === a(this).find(".tile-white-is-selected").length) return void a(this).find(".content").hide();
            if (a(this).find(".content").show(), !a(this).parent().hasClass("filler")) {
                a(this).addClass("remove-background"), a(this).find(".streamium-extra-meta").hide();
        ...
        }), (streamium_object.is_home || streamium_object.is_archive && !streamium_object.is_tax) && c.getData({
            action: "home_api_post",
            nonce: streamium_object.home_api_nonce
        }, function (b) {
            if (b.error) return void console.log("Error: ", b.message);
            c.homeTemplate(b, function () {
                var b = a(".carousels").height() / 2;
                a(".video-header").height(b), a(".streamium-loading").fadeOut(), a(".lazy").Lazy(c.lazy)
            })
        }), streamium_object.is_home && (c.getData({
            action: "recently_watched_api_post",
            nonce: streamium_object.recently_watched_api_nonce
        }, function (a) {
            if (a.error) return void console.log("Error: ", a.message);
            c.recentTemplate(a, function () {
            })
        }), c.getData({action: "custom_api_post", nonce: streamium_object.custom_api_nonce}, function (a) {
            if (a.error) return void console.log("Error: ", a.message);
            c.customTemplate(a, function () {
            })
        })), streamium_object.is_tax && (c.getData({
            action: "tax_api_post",
            search: "all",
            ...
            b.id = "tax-watched", c.staticTemplate(b, function () {
                var b = a(".tile").height() / 2;
                a(".static-row").css("margin-top", b + "px"), a(".lazy").Lazy(c.lazy)
            })
            ...
            if (b.error) return console.log("Error: ", b.message), void a("#search-watched").html('<div class="container-fluid"><div class="row"><div class="col-sm-12">Error: ' + b.message + "...</div></div></div>");
            ...
            b.preventDefault();
            var d = a(this).data("type");
            streamium_object.search = {s: "all", date: d}, c.getData({
                action: "search_api_post",
                nonce: streamium_object.search_api_nonce
            }, function (b) {
                if (b.error) return console.log("Error: ", b.message), void a("#search-watched").html('<div class="container-fluid"><div class="row"><div class="col-sm-12">Error: ' + b.message + "...</div></div></div>");
...
+function (a) {
...
}(jQuery), window.FontAwesomeCdnConfig = {
    autoA11y: {enabled: !1},
    ...
    function c(c, d, f, h, i) {
        ...
        function k(a) {
            var f = d.defaultImage, g = d.placeholder, h = d.imageBase, i = d.srcsetAttribute, j = d.loaderAttribute,
                k = d._f || {};
            a = e(a).filter(function () {
                var a = e(this), c = q(this);
                return !a.data(d.handledName) && (a.attr(d.attribute) || a.attr(i) || a.attr(j) || k[c] !== b)
            }).data("plugin_" + d.name, c);
            for (var l = 0, m = a.length; l < m; l++) {
                var n = e(a[l]), o = q(a[l]), p = n.attr(d.imageBaseAttribute) || h;
        ...
        function l(a, b) {
            ...
            for (var g = b || f, h = !1, i = d.imageBase || "", j = d.srcsetAttribute, k = d.handledName, l = 0; l < g.length; l++) if (a || b || n(g[l])) {
                var o = e(g[l]), p = q(g[l]), r = o.attr(d.attribute), s = o.attr(d.imageBaseAttribute) || i,
        ...
        function m(a, b, c, f) {
            ...
            u("beforeLoad", a);
            var h = d.attribute, i = d.srcsetAttribute, j = d.sizesAttribute, k = d.retinaAttribute,
                l = d.removeAttribute, m = d.loadedName, n = a.attr(k);
            if (f) {
                var o = function () {
        ...
        function n(a) {
            var b = a.getBoundingClientRect(), c = d.scrollDirection, e = d.threshold,
                f = p() + e > b.top && -e < b.bottom, g = o() + e > b.left && -e < b.right;
        ...
        var v = 0, w = -1, x = -1, y = !1, z = "afterLoad", A = "load", B = "error", C = "img", D = "src", E = "srcset",
            F = "sizes", G = "background-image";
        "event" === d.bind || g ? j() : e(a).on(A + "." + i, j)
    }

    function d(d, g) {
        var h = this, i = e.extend({}, h.config, g), j = {}, k = i.name + "-" + ++f;
    ...
    var e = a.jQuery || a.Zepto, f = 0, g = !1;
    ...
    function d() {
        ...
        a(".info-quick-view").css({top: c, left: b})
    }

    function e(b, c) {
        var d = a(".info-close"), e = d.siblings(".info-slider-wrapper").find(".selected img").attr("src"),
    ...
    function f(b, c, d, e) {
        var f = b.parent(".info-item"), g = b.offset().top - a(window).scrollTop(), h = b.offset().left, i = b.width(),
            j = b.height(), k = a(window).width(), l = a(window).height(), m = (k - c) / 2, n = c * j / i,
            o = (l - n) / 2, p = .8 * k < d ? .8 * k : d, q = (k - p) / 2;
        "open" == e ? (f.addClass("empty-box"), a(".info-quick-view").css({
            top: g,
            left: h,
            width: i
        }).velocity({top: o + "px", left: m + "px", width: c + "px"}, 1e3, [400, 20], function () {
    ...
    function g(b, c, d) {
        var e = b.parent(".info-item"), f = b.offset().top - a(window).scrollTop(), g = b.offset().left, h = b.width();
    ...
    a(".info-trigger").on("click", function (b) {
        var d = a(this).parent(".info-item").children("img"), e = d.attr("src");
    ...
    (function (a, b, c, d) {
        ...
        function f(b, c) {
            ...
            if (o.search(/mouse(down|up)|click/) > -1 && !b.which && (b.which = 1), -1 !== o.search(/^touch/) && (h = e(f), o = h.touches, j = h.changedTouches, k = o && o.length ? o[0] : j && j.length ? j[0] : d, k)) for (m = 0, n = B.length; m < n; m++) i = B[m], b[i] = k[i];
            return b
        }
        ...
        var w, x, y = "virtualMouseBindings", z = "virtualTouchID",
            ...
            C = a.event.mouseHooks ? a.event.mouseHooks.props : [], D = a.event.props.concat(C), E = {}, F = 0, G = 0,
            H = 0, I = !1, J = [], K = !1, L = !1, M = "addEventListener" in c, N = a(c), O = 1, P = 0;
        for (a.vmouse = {
        ...
        M && c.addEventListener("click", function (b) {
            var c, d, e, f, g, h = J.length, i = b.target;
            if (h) for (c = b.clientX, d = b.clientY, w = a.vmouse.clickDistanceThreshold, e = i; e;) {
                for (f = 0; f < h; f++) if (g = J[f], 0, e === i && Math.abs(g.x - c) < w && Math.abs(g.y - d) < w || a.data(e, z) === g.touchID) return b.preventDefault(), void b.stopPropagation();
                e = e.parentNode
            }
        }, !0)
    })(a, 0, c), function (a) {
    ...
    e(), a(window).on("resize", function () {
        ...
        b(), c("close"), a(".cd-overlay").removeClass("is-visible")
    }), a(".cd-primary-nav").children(".has-children").children("a").on("click", function (a) {
        a.preventDefault()
    }), a(".has-children").children("a").on("click", function (b) {
        d() || b.preventDefault();
        var e = a(this), f = e.parent().attr("id");
        if (e.next("ul").hasClass("is-hidden")) {
            e.addClass("selected").next("ul").removeClass("is-hidden").end().parent(".has-children").parent("ul").addClass("moves-out");
            var g = a("#" + f + " ul").children().length;
            a("#" + f).animate({height: 30 * g}, 200), e.parent(".has-children").siblings(".has-children").children("ul").addClass("is-hidden").end().children("a").removeClass("selected"), a(".cd-overlay").addClass("is-visible")
        } else a("#" + f).animate({height: 0}, 200), a("#" + f).css("height", "auto"), e.removeClass("selected").next("ul").addClass("is-hidden").end().parent(".has-children").parent("ul").removeClass("moves-out"), a(".cd-overlay").removeClass("is-visible");
        c("close")
    }), a(".go-back").on("click", function () {
        var b = a(this), c = b.parents("li:eq(0)").attr("id");
        a("#" + c).css("height", "auto"), a(this).parent("ul").addClass("is-hidden").parent(".has-children").parent("ul").removeClass("moves-out")
    })
}), jQuery(document).ready(function (a) {
    function b(b) {
        var d = b.width() / 2, e = b.offset().left + d, f = b.offset().top + d - a(window).scrollTop(),
            g = c(f, e, d, a(window).height(), a(window).width());
        return b.css("position", "fixed").velocity({top: f - d, left: e - d, translateX: 0}, 0), g
    }

    function c(a, b, c, d, e) {
        var f = b > d / 2 ? b : d - b, g = a > e / 2 ? a : e - a;
        return Math.ceil(Math.sqrt(Math.pow(f, 2) + Math.pow(g, 2)) / c)
    }

    function d(b, c, d) {
        b.velocity({scale: c}, 400, function () {
    ...
    function e() {
        var b = a(".modal-section.modal-is-visible").find(".modal-modal-bg"), d = b.width() / 2,
            e = b.siblings(".btn").offset().top + d - a(window).scrollTop(), f = b.siblings(".btn").offset().left + d,
            g = c(e, f, d, a(window).height(), a(window).width());
        b.velocity({top: e - d, left: f - d, scale: g}, 0)
    }
    ...
    a('[data-type="modal-trigger"]').on("click", function () {
        var c = a(this), e = b(c.next(".modal-modal-bg"));
    ...
    var k, l, m = {}, n = b.documentElement, o = "modernizr", p = b.createElement(o), q = p.style,
        r = b.createElement("input"), s = ":)", t = {}.toString, u = " -webkit- -moz- -o- -ms- ".split(" "),
        v = "Webkit Moz O ms", w = v.split(" "), x = v.toLowerCase().split(" "),
        y = {svg: "http://www.w3.org/2000/svg"}, z = {}, A = {}, B = {}, C = [], D = C.slice,
        E = function (a, c, d, e) {
            var f, g, h, i, j = b.createElement("div"), k = b.body, l = k || b.createElement("body");
            if (parseInt(d, 10)) for (; d--;) h = b.createElement("div"), h.id = e ? e[d] : o + (d + 1), j.appendChild(h);
...
}(this, this.document), jQuery(document).ready(function (a) {
    a(".streamium-list-reviews").live("click", function (b) {
        ...
        var c = a(this).attr("data-id"), d = a(this).attr("data-nonce");
        a.ajax({
            url: streamium_object.ajax_url,
            type: "post",
            dataType: "json",
            data: {action: "streamium_get_reviews", post_id: c, nonce: d},
            success: function (b) {
                if (b.error) return void swal({
                    title: "Error",
                    text: b.message,
                ...
                a.each(b.data, function (a, b) {
                    var d = b.avatar, e = b.message, f = b.time, g = b.username;
                ...
                }), c += "</ul>", a(".streamium-review-panel-content").html(c), a(".streamium-review-panel-header h1").html(b.title + " Reviews"), a(".streamium-review-panel").addClass("is-visible")
            }
        })
    }), a(".streamium-review-panel").live("click", function (b) {
        (a(b.target).is(".streamium-review-panel") || a(b.target).is(".streamium-review-panel-close")) && (a(".streamium-review-panel").removeClass("is-visible"), b.preventDefault())
    }), a(".streamium-review-panel-close").live("click", function (b) {
        a(".streamium-review-panel").removeClass("is-visible"), b.preventDefault()
        ...
        return swal({
            ...
            return !1 !== b && ("" === b || b.length < 100 ? (swal.showInputError("Please enter over 100 characters!"), !1) : void a.ajax({
                ...
                type: "post",
                dataType: "json",
                data: {action: "streamium_likes", post_id: c, message: b, nonce: d},
                success: function (b) {
                    if (b.error) return void swal({
                        title: "Error",
                        text: b.message,
    ...
    a(".video-player-streaming")[0] && function () {
        video_post_object.youtube ? s3bubble("s3bubble-" + video_post_object.post_id).service({
            codes: video_post_object.codes,
            startTime: video_post_object.percentage,
            source: {poster: video_post_object.poster},
            options: {autoplay: !0, fluid: !0},
            meta: {backButton: !0, subTitle: video_post_object.subTitle, title: video_post_object.title, para: ""},
            brand: {
            ...
            b.on("timeupdate", function () {
                var a = this.currentTime(), b = this.duration(), c = a / b * 100;
            ...
            startTime: video_post_object.percentage,
            source: {poster: video_post_object.poster},
            options: {autoplay: !0, fluid: !0, vpaid: video_post_object.vpaid},
            meta: {backButton: !0, subTitle: video_post_object.subTitle, title: video_post_object.title, para: ""},
            brand: {
                controlbar: video_post_object.brand_control,
                icons: video_post_object.brand_icons,
    ...
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && (b = !0), a(".streamium-uploader")[0] && function () {
        0 !== streamium_uploader.length && new plupload.Uploader({
            ...
            filters: {
                ...
                mime_types: [{title: "Allowed files", extensions: streamium_uploader.filetypes}]
            },
            init: {
                PostInit: function () {
    ...
    function b(b) {
        var c = a(this), d = null, e = [], f = null, g = null, h = a.extend({
            rowSelector: "> li",
            submenuSelector: "*",
            ...
            var g = c.offset(), i = {x: g.left, y: g.top - h.tolerance}, j = {x: g.left + c.outerWidth(), y: i.y},
                k = {x: g.left, y: g.top + c.outerHeight() + h.tolerance}, l = {x: g.left + c.outerWidth(), y: k.y},
                m = e[e.length - 1], n = e[0];
            if (!m) return 0;
            ...
            var q = b(m, o), r = b(m, p), s = b(n, o), t = b(n, p);
    ...
    b = function () {
        ...
        if (!0 === a.options.infinite) for (; b < a.slideCount;) ++d, b = c + a.options.slidesToScroll, c += a.options.slidesToScroll <= a.options.slidesToShow ? a.options.slidesToScroll : a.options.slidesToShow; else if (!0 === a.options.centerMode) d = a.slideCount; else if (a.options.asNavFor) for (; b < a.slideCount;) ++d, b = c + a.options.slidesToScroll, c += a.options.slidesToScroll <= a.options.slidesToShow ? a.options.slidesToScroll : a.options.slidesToShow; else d = 1 + Math.ceil((a.slideCount - a.options.slidesToShow) / a.options.slidesToScroll);
        return d - 1
        ...
        var b = this
        ;!0 === b.options.dots && b.slideCount > b.options.slidesToShow && a("li", b.$dots).on("click.slick", {message: "index"}, b.changeSlide), !0 === b.options.dots && !0 === b.options.pauseOnDotsHover && a("li", b.$dots).on("mouseenter.slick", a.proxy(b.interrupt, b, !0)).on("mouseleave.slick", a.proxy(b.interrupt, b, !1))
    ...
    !function a(b, c, d) {
        ...
        1: [function (d, e, f) {
            ...
            var h, i, j, k, l = d("./modules/handle-dom"), m = d("./modules/utils"), n = d("./modules/handle-swal-dom"),
                o = d("./modules/handle-click"), p = d("./modules/handle-key"), q = g(p),
                r = d("./modules/default-params"), s = g(r), t = d("./modules/set-params"), u = g(t);
            f.default = j = k = function () {
                ...
                }, v = 0; v < p.length; v++) for (var w = 0; w < r.length; w++) {
                    var x = r[w];
                    p[v][x] = t
                }
                ...
                var b = (0, n.getModal)(), c = b.querySelector(".sa-input-error");
                (0, l.addClass)(c, "show");
                ...
                var b = (0, n.getModal)(), c = b.querySelector("button.confirm"), d = b.querySelector("button.cancel");
            ...
            var e = b("./utils"), f = (b("./handle-swal-dom"), b("./handle-dom")), g = function (b, c, d) {
                ...
                var j, k, l, m = b || a.event, n = m.target || m.srcElement, o = -1 !== n.className.indexOf("confirm"),
                    p = -1 !== n.className.indexOf("sweet-overlay"), q = (0, f.hasClass)(d, "visible"),
                ...
                switch (o && c.confirmButtonColor && (j = c.confirmButtonColor, k = (0, e.colorLuminance)(j, -.04), l = (0, e.colorLuminance)(j, -.14)), m.type) {
                    ...
                    case"mouseup":
                        ...
                        break;
                    case"focus":
                        var s = d.querySelector("button.confirm"), t = d.querySelector("button.cancel");
                        o ? t.style.boxShadow = "none" : s.style.boxShadow = "none";
                        break;
                    case"click":
                        var u = d === n, v = (0, f.isDescendant)(d, n);
        ...
        }, {"./handle-dom": 4, "./handle-swal-dom": 6, "./utils": 9}], 4: [function (c, d, e) {
        ...
        }, {"./handle-dom": 4, "./handle-swal-dom": 6}], 6: [function (c, d, e) {
            function f(a) {
                return a && a.__esModule ? a : {default: a}
            }

            Object.defineProperty(e, "__esModule", {value: !0});
            var g = c("./utils"), h = c("./handle-dom"), i = c("./default-params"), j = f(i), k = c("./injected-html"),
                l = f(k), m = function () {
                    var a = b.createElement("div");
                    ...
                    if (a && 13 === a.keyCode) return !1;
                    var b = n(), c = b.querySelector(".sa-input-error");
        ...
        }, {"./default-params": 2, "./handle-dom": 4, "./injected-html": 7, "./utils": 9}], 7: [function (a, b, c) {
            Object.defineProperty(c, "__esModule", {value: !0});
            ...
            Object.defineProperty(d, "__esModule", {value: !0});
            var e = a("./utils"), f = a("./handle-swal-dom"), g = a("./handle-dom"),
                h = ["error", "warning", "info", "success", "input", "prompt"], i = function (a) {
                    var b = (0, f.getModal)(), d = b.querySelector("h2"), i = b.querySelector("p"),
                    ...
                    if ((0, g.hide)(b.querySelectorAll(".sa-icon")), a.type && !(0, e.isIE8)()) {
                        var m = function () {
                            for (var d = !1, e = 0; e < h.length; e++) if (a.type === h[e]) {
                                d = !0;
                                break
                            }
                            if (!d) return logStr("Unknown alert type: " + a.type), {v: !1};
                            var i = ["success", "error", "warning", "info"], j = c;
                            -1 !== i.indexOf(a.type) && (j = b.querySelector(".sa-icon.sa-" + a.type), (0, g.show)(j));
                            var k = (0, f.getInput)();
                            switch (a.type) {
                                case"success":
        ...
        }, {"./handle-dom": 4, "./handle-swal-dom": 6, "./utils": 9}], 9: [function (b, c, d) {
            Object.defineProperty(d, "__esModule", {value: !0});
            var e = function (a, b) {
